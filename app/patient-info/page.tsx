...
export default function PatientInfoPage() {
    ...
    const urlPatientLetter = searchParams?.get('patientLetter');
    const [incompleteSections, setIncompleteSections] = useState<string[]>([]);
    const [patientLetter, setPatientLetter] = useState(urlPatientLetter || 'A');

    // Load patient letter on mount (from URL if present)
    useEffect(() => {
      if (incidentId && urlPatientLetter) {
        setPatientLetter(urlPatientLetter);
      } else if (incidentId) {
        setPatientLetter(getCurrentPatientLetter(incidentId));
      }
    }, [incidentId, urlPatientLetter]);
    ...
    const confirmAddPatient = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          setTimeout(() => {
            const params = new URLSearchParams({ id: incidentId, fleetId, patientLetter: result.newLetter });
    ...
    // Admin Panel removed from patient-info page
    ...
    return (
      <div className="eprf-dashboard incident-page">
        <div className="eprf-nav">
          ...
          {/* Admin Panel button removed from patient-info page */}
        ...
        <PatientManagementModal
          ...
          onPatientSwitch={(letter) => {
            ...
            const params = new URLSearchParams({ id: incidentId, fleetId, patientLetter: letter });
            router.push(`/patient-info?${params}`);
          }}
          onPatientAdded={(newLetter, previousLetter) => {
            ...
            setShowSuccessModal(true);
            setTimeout(() => {
              const params = new URLSearchParams({ id: incidentId, fleetId, patientLetter: newLetter });
              router.push(`/patient-info?${params}`);
            }, 2000);
          }}
        />
